    ...
    async def process_message(self, message_data: Dict[str, Any]):
        ...
        try:
            ...
            if message.type == MessageType.DELEGATION:
                ...
                # Handle error messages from child services
                logger.error(f"Received error from {message.source}: {message.content}")
                # If we have a pending response for this correlation_id, update it
                if message.correlation_id in self.pending_responses:
                    self.pending_responses[message.correlation_id]["branch_responses"][message.source] = f"Error: {message.content}"
                    # Check if we have all responses (including errors)
                    if len(self.pending_responses[message.correlation_id]["branch_responses"]) >= 2:
